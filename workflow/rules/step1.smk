...
rule figaro:
    input:
        seqkit_files="data/cutadapt/{batch}-{region}/.trim_R2.done"
    output:
        "data/cutadapt/{batch}-{region}/figaro/trimParameters.json"
    log:
    ...
    shell:
        """
         python workflow/envs/figaro/figaro/figaro.py -i data/cutadapt/{wildcards.batch}-{wildcards.region}/cutadapt/trim -o {output} -f 1 -r 1 -a 500 -F illumina >> {log}
        """
...
rule dada2:
    input:
        "data/cutadapt/{batch}-{region}/figaro/trimParameters.json"
    output:
        "data/cutadapt/{batch}-{region}/cutadapt/seqtab.tsv"
    log:
        R1="data/logs/figaro-{batch}-{region}.log"
    conda:
        "../envs/dada2.yaml"
    ...
    shell:
        """
         Rscript --vanilla workflow/scripts/dada2.R -i {input} -f data/cutadapt/{wildcards.batch}-{wildcards.region}/cutadapt -p {params.figaro} > {log}
        """
...
rule all:
    input:
        expand("data/cutadapt/{batch}-{region}/cutadapt/seqtab.tsv", 
