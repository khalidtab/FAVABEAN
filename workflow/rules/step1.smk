...
# Path to the environments log file
environments_log = "environments.txt"
output_csv = "data/files_info_Batches.csv"
env_path = None

# Step 1: Extract the conda environment path for `dada2.yaml` from `environments.txt`
with open(environments_log, 'r') as f:
    content = f.read()
    # Search for the environment path for dada2.yaml
    match = re.search(r"Environment for /./workflow/envs/dada2.yaml created \(location: (.+?)_\)", content)
    if match:
        env_path = match.group(1) + "_"
    else:
        print("Error: Could not find the conda environment path for dada2.yaml.")
        exit(1)
...
if not os.path.exists(output_csv):
    try:
        ...
        subprocess.run(
            f"conda run -p {env_path} Rscript workflow/scripts/batch.R",
